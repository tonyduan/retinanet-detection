...
if __name__ == "__main__":
    ...
    new_state_dict = {}
    for my_key, ref_key in zip(filter(lambda k: "shortcut" not in k, my_model.state_dict().keys()), 
                               filter(lambda k: "downsample" not in k, ref_model.state_dict().keys())):
        if ref_key == "fc.weight":
            break
        new_state_dict[my_key] = pretrained_state_dict[ref_key]

    for my_key, ref_key in zip(filter(lambda k: "shortcut" in k, self.state_dict().keys()),
